...
parser = argparse.ArgumentParser(
    ...
    epilog=f'If you have any questions, please feel free to reach out to {CAA_EMAIL}',
    add_help=False)
...
def request_post_api(list_json_payload, db_table_name, route_url):
    ...
    api_auth = HTTPBasicAuth(CAA_REST_API_USER, CAA_REST_API_KEY)
    api_url = f'{CAA_URL_BASE}/{route_url}'
...
def test_supabase_action_flow_core():
    ...
    print_info(
        f'[INFO] Your dashboard is now updated at {CAA_URL_BASE + "/dashboard"}')
    if not is_yes:
        messagebox.showinfo(
            title='CAA', message=f'Your dashboard is now updated at {CAA_URL_BASE + "/dashboard"}')
        exit_program()
    ...
    if NAME_RLS in list_findings_discovered:
        ...
        for project_id_fk in list_table_project_id_fk_rls_disabled:
            for i in range(len(list_table_project_id_fk_rls_disabled)):
                ...
                try:
                    ...
                    print_error(f"[ERROR] {NAME_RLS} couldn't be enabled for the table {table_name}. Please check the logs and contact us <{CAA_EMAIL}>.",
                                project_id=table_project_id_fk,
                                table_project_id=table_project_id_fk,
                                table_name=table_name)
        ...
        if is_all_rls_remediated:
            ...
            message = f"{NAME_RLS} remediation process has failed. Please check the logs and contact us <{CAA_EMAIL}>."
